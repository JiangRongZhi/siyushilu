...
class Caption {
  constructor(content, capOpt = {}) {
    this.content = content
    this.color = capOpt.color || '#ffffff'
    this.duration = capOpt.duration || 3
    this.interval = capOpt.interval || 40
    this.startTime = capOpt.startTime || 0
    this.left = capOpt.left || 0
    this.top = capOpt.top || 50
    this.step = this.left / (this.duration * (1000 / this.interval))
...
export default class DanmuPlayer {
  // 构造
  // options:
  constructor(options) {
    this.__props__(options)
    this.__init__()
    this.instance = null
  }
  ...
  static getInstance(options) {
    if (!this.instance) {
      this.instance = new DanmuPlayer(options)
    }
  ...
  // 初始化
  __init__() {
    this.cav.width = this.options.width
    this.cav.height = this.options.height
    this.update()
    // this.videoImage()
  }

  update() {
    // { ctx, width, height } = this.options
    const ctx = this.ctx
    ...
    this._timer = setInterval(() => {
      ...
      ctx.fillText('time:' + time, 0, 15)
      this.captions.forEach((item, index) => {
        // console.log(item, index)
        ctx.font = 'bold 20px Courier New'
        if (time > item.startTime) {
          ctx.fillStyle = item.color
          ctx.fillText(item.content, item.left, item.top)
          ctx.closePath()
          if (!item.move()) {
...
/*
const videoPlayer = function(cav, video) {
  // 设置常量canvas的高度以及宽度
  const cavWidth = 600
  const cavHeight = 400
  cav.width = cavWidth
  ...
  const cav1 = document.createElement('canvas')
  cav1.width = cavWidth
  cav1.height = cavHeight
  ...
  function computeFrame() {
    ctx1.drawImage(video, 0, 0, cavWidth, cavHeight)
    // const frame = ctx1.getImageData(0, 0, cavWidth, cavHeight)
  ...
  danmuCanvas.width = cavWidth
  danmuCanvas.height = cavHeight
  const danmuCtx = cav1.getContext('2d')
  danmuCtx.font = '20px Microsoft YaHei'

  setInterval(() => {
    danmuCtx.clearRect(0, 0, cavWidth, cavHeight)
    danmuCtx.save()

    danmuCtx.fillStyle = '#000000'
    danmuCtx.fillText('canvas 绘制文字', 10, 10)
    const danmuFrame = danmuCtx.getImageData(0, 0, cavWidth, cavHeight)

    const frame = ctx1.getImageData(0, 0, cavWidth, cavHeight)
    ctx.putImageData(frame, 0, 0)
    ctx.putImageData(danmuFrame, 0, 0)
  }, 40)
  // console.log(_timer)const _timer =
